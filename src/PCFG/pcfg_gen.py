...
class PCFGGenerator:
    ...
    def _expand_symbol(self, symbol: str) -> list[str]:
        ...
        probs, expansions = zip(*self.rules[symbol], strict=False)
        return random.choices(expansions, weights=probs)[0]
    ...
    **Correlated** strings satisfy ``count('a') == count('b') + 1`` in the
    ...
    while len(correlated) < n_correlated:
        ...
        if verbose and total_generated % 50_000 == 0:
            len(correlated) / total_generated
    ...
    if verbose:
        len(correlated) / max(total_generated, 1)
...
class TaskRegistry:
    ...
    def register(self, name: str, task_fn: Callable[[str], tuple[str, str]]) -> None:
        """Register a new task.
    ...
    def get_task(self, name: str) -> Callable[[str], tuple[str, str]]:
        ...
        if name not in self.tasks:
            msg = f"Task '{name}' not found. Available: {list(self.tasks.keys())}"
            raise ValueError(msg)
...
class CharTokenizer:
    ...
    def __init__(self) -> None:
        NUMERIC_TOKENS = [str(i) for i in range(251)] + ["-1"]
        vocab_tokens = [
                        # task family tokens
                        "C", "I", "T", "N", "U", "L",
        ...
        self.idx2tok = dict(enumerate(vocab))
...
class PCFGDataset(Dataset):
    ...
    def __init__(self, examples: list[list[str]], tokenizer: CharTokenizer, max_length: int = 512, mask_answer_only: bool = True) -> None:

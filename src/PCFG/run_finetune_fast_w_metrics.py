...
from train_help import _record_phase, build_eval_datasets, train
...
def make_on_eval(correlation, prev_state_holder):
    ...
    def on_eval(model, step, history) -> None:
        # ---- Gradient projection ----
        for task_a, task_b in TASK_PAIRS:
            ...
            ", ".join(f"L{l}:{proj[l]['cosine_sim']:.3f}" for l in sorted(proj))
            with print_lock:
                pass
        ...
        ", ".join(f"L{l}:{drift[l]:.4f}" for l in range(n_layers))
        with print_lock:
            pass

...
import torch
from pcfg_gen import PCFGDataset, collate_fn, format_example
...
def sample_batch(batch_size, task_names, task_weights, pcfg_gen, task_reg,
    ...
    return collate_fn([batch_dataset[i] for i in range(len(batch_dataset))], tokenizer)
...
def build_task_weights(task_names: list[str], operand_probs: dict[str, float]) -> list[float]:
    ...
    for name in task_names:
        if (name.startswith("count_") and name in ["count_a", "count_b", "count_c"]) or (name.startswith("index_") and name in ["index_a", "index_b", "index_c"]):
            tok = name.split("_")[1]

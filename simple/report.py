...
def plot_accuracy(ft_results, fg_results, figsize=(14, 5)):
    ...
    for ax, title in [(ax_burst, "Burst Accuracy"), (ax_other, "Background Accuracy")]:
...
def plot_loss(ft_results, fg_results, figsize=(14, 5)):
    ...
    for ax, title in [(ax_burst, "Burst Loss"), (ax_other, "Background Loss")]:
        ax.set_xlabel("Step (finetune | forget)")
        ax.set_ylabel("Loss")
        ax.set_title(title)
        ax.legend(fontsize=7)
...
def plot_weight_drift(ft_results, fg_results, figsize=(14, 5)):
    ...
    for ft, fg in _pair_ft_fg(ft_results, fg_results):
        ...
        if "weight_drift" not in ft_log:
            continue
        ax_pt.plot(
            ft_log["step"], ft_log["weight_drift"], color=color, linewidth=2, label=ft["tag"]
        )
        if fg is not None:
            fg_log = fg["log"]
            if fg_log.get("weight_drift_from_pt"):
                fg_steps = _offset_fg_steps(ft_log, fg_log)
                ax_pt.plot(fg_steps, fg_log["weight_drift_from_pt"], color=color, linewidth=2)
    ...
    for ft, fg in _pair_ft_fg(ft_results, fg_results):
        ...
        color = _frac_color(ft["burst_frac"])
        fg_log = fg["log"]
        if fg_log.get("weight_drift_from_ft"):
            ax_ft.plot(
                fg_log["step"],
                fg_log["weight_drift_from_ft"],
                color=color,
                linewidth=2,
                label=ft["tag"],
            )
...
def plot_grad_norms(ft_results, fg_results, figsize=(14, 8)):
    ...
    for r in ft_list:
        log = r["log"]
        if "grad_norm_burst" not in log:
            continue
        ax.plot(
            log["step"],
            _smooth(log["grad_norm_burst"]),
    ...
    for ft, fg in _pair_ft_fg(ft_results, fg_results):
        color = _frac_color(ft["burst_frac"])
        ft_log = ft["log"]
        if "grad_norm_bg" not in ft_log:
            continue
        ax.plot(
            ft_log["step"],
            _smooth(ft_log["grad_norm_bg"]),
            color=color,
            linewidth=1.5,
            label=ft["tag"],
        )
    ...
    for ft, fg in _pair_ft_fg(ft_results, fg_results):
        color = _frac_color(ft["burst_frac"])
        ft_log = ft["log"]
        if "grad_norm_train" not in ft_log:
            continue
        ax.plot(
            ft_log["step"],
            _smooth(ft_log["grad_norm_train"]),
            color=color,
            linewidth=1.5,
            label=ft["tag"],
        )
...
def plot_grad_cosine(ft_results, fg_results, figsize=(12, 5)):
    ...
    for ft, fg in _pair_ft_fg(ft_results, fg_results):
        ...
        ax.plot(
            ft_log["step"],
            _smooth(ft_log["grad_cosine_burst_bg"]),
            color=color,
            linewidth=2,
            label=ft["tag"],
        )
        if fg is not None:
            fg_log = fg["log"]
            if fg_log.get("grad_cosine_burst_bg"):
                fg_steps = _offset_fg_steps(ft_log, fg_log)
                ax.plot(fg_steps, _smooth(fg_log["grad_cosine_burst_bg"]), color=color, linewidth=2)
...
def _build_layer_cosine_matrix(results):
    ...
    matrix = np.array([[rec.get(layer_name, 0.0) for layer_name in layer_names] for rec in records])
...
def plot_grad_norm_entropy(ft_results, fg_results, figsize=(14, 5)):
    ...
    for ft, fg in _pair_ft_fg(ft_results, fg_results):
        ...
        if ft_log.get("grad_norm_entropy_burst"):
            ax_burst.plot(
                ft_log["step"],
                _smooth(ft_log["grad_norm_entropy_burst"]),
                color=color,
                linewidth=2,
                label=ft["tag"],
            )
        ...
        if ft_log.get("grad_norm_entropy_bg"):
            ax_bg.plot(
                ft_log["step"],
                _smooth(ft_log["grad_norm_entropy_bg"]),
                color=color,
                linewidth=2,
                label=ft["tag"],
            )
        if fg is not None:
            fg_log = fg["log"]
            if fg_log.get("grad_norm_entropy"):
                fg_steps = _offset_fg_steps(ft_log, fg_log)
                ax_bg.plot(fg_steps, _smooth(fg_log["grad_norm_entropy"]), color=color, linewidth=2)
...
def plot_grad_cosine_per_layer(ft_results, fg_results, figsize_per=(14, 3.5)):
    ...
    fig, axes = plt.subplots(
        n_rows, 1, figsize=(figsize_per[0], figsize_per[1] * n_rows), squeeze=False
    )
    vmin, vmax = -1, 1
    ...
    for i, (ft, fg) in enumerate(pairs):
        ...
        if built_fg is not None:
            ax.axvline(steps_ft[-1], color="black", linewidth=1.5, linestyle="--", alpha=0.6)

        frac = ft["burst_frac"]
        ax.set_title(f"{ft['tag']} ({frac * 100:.0f}%)", fontsize=10)
        ax.set_xlabel("Step (finetune | forget)")

    fig.suptitle("Per-Block Gradient Cosine (Burst vs Background)", fontsize=13, fontweight="bold")
    fig.tight_layout()
    return fig
...
def plot_svd_analysis(analysis, phase="pt_ft", figsize=(14, 5)):
    ...
    for tag in tags:
        ...
        ax1.plot(
            range(len(layers)),
            [svd[layer_name]["effective_rank"] for layer_name in layers],
            color=color,
            label=tag,
            marker="o",
            markersize=4,
        ...
        ax2.plot(
            range(len(layers)),
            [svd[layer_name]["spectral_norm"] for layer_name in layers],
            color=color,
            label=tag,
            marker="s",
            markersize=4,
...
def plot_summary_dashboard(ft_results, fg_results, analysis, figsize=(16, 10)):
    ...
    for t in tags:
        if t in analysis and analysis[t].get("svd_pt_ft"):
            mean_ranks.append(
                np.mean([v["effective_rank"] for v in analysis[t]["svd_pt_ft"].values()])
            )
        else:
            mean_ranks.append(0)
    ...
    for r in ft_list:
        log = r["log"]
        if log.get("grad_cosine_burst_bg"):
...
def plot_function_distribution(data, figsize=None):
    ...
    for slot_idx in range(depth):
        ...
        ax.bar(
            x - w / 2,
            [bg_counts.get(f, 0) for f in all_fns],
            w,
            label="Pretrain (bg)",
            color="#2196F3",
            alpha=0.8,
        ...
        ax.bar(
            x + w / 2,
            [ft_counts.get(f, 0) for f in all_fns],
            w,
            label="Finetune (burst)",
            color="#E91E63",
            alpha=0.8,
        )
    ...
    fig.suptitle(
        "Function Distribution per Slot: Pretrain vs Finetune", fontsize=13, fontweight="bold"
    )
    fig.tight_layout()
    return fig

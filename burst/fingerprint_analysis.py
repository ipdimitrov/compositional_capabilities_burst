...
def steering_sweep_layers(
    ...
    for k in range(K):
        result = steering_experiment(
            net, delta_KTN, other_docs_BL, burst_docs_BL, prompt_len,
            steer_layer=k, alphas=[alpha], n_samples=n_samples,
        )
...
def analyse_run(
    ...
    for sched in schedules_present:
        ...
        for r in sched_results:
            ...
            print(f"    Logit Lens readability (with LN): "
                  f"mean={ll_result['with_ln']['readability_KT'].mean():.3f}", flush=True)

            # --- Steering (best layer = middle) ---
            K = delta_KTN.shape[0]
            mid_layer = K // 2
            steer_result = steering_experiment(
                net_peak, delta_KTN, other_docs_BL, burst_docs_BL, prompt_len,
                steer_layer=mid_layer, alphas=steering_alphas, n_samples=min(n_samples, 128),
            )
            ...
            sweep = steering_sweep_layers(
                net_peak, delta_KTN, other_docs_BL, burst_docs_BL, prompt_len,
                alpha=5.0, n_samples=min(n_samples, 64),
            )
        ...
        analysis["logit_lens"][sched] = {
            method: {
                ...
                "overall_readability": float(np.mean([a.mean() for a in ll_readability_agg[method]])),
...
_METRIC_DESCRIPTIONS = {
    ...
    "logit_lens_entropy": {
        ...
        "low": "Concentrated distribution — δ̄ points sharply at specific tokens (strong fingerprint).",
    },
...
def make_dashboard(analyses: list[dict], out_dir: Path) -> None:
        ...
        for method, method_label in [("with_ln", "with LayerNorm"), ("without_ln", "raw (no LN)")]:
            ...
            for i, sched in enumerate(schedules):
                ...
                fig.add_trace(go.Heatmap(
                    ...
                    colorscale="YlOrRd",
                    zmin=0, zmax=1,
                    colorbar=dict(title="Readability", len=1/len(schedules), y=1 - i/len(schedules)),
                    showscale=(i == 0),
                ), row=i+1, col=1)
            ...
            _add_fig(f"logit_lens_readability_heatmap_{method}_{run_name}", fig,
    ...
    for analysis in analyses:
        ...
        _add_fig(f"logit_lens_comparison_bar_{run_name}", fig,
    ...
    for analysis in analyses:
        ...
        for i, sched in enumerate(schedules):
            ...
            fig.add_trace(go.Heatmap(
                ...
                colorbar=dict(title="Mean Rank", len=1/len(schedules), y=1 - i/len(schedules)),
                showscale=(i == 0),
            ), row=i+1, col=1)
            fig.update_yaxes(title_text="Layer", row=i+1, col=1)
        ...
        _add_fig(f"logit_lens_mean_rank_{run_name}", fig,
    ...
    for analysis in analyses:
        ...
        for i, sched in enumerate(schedules):
            ...
            fig.add_trace(go.Heatmap(
                ...
                y=[f"L{k}" for k in range(K)],
                colorscale="Blues",
                colorbar=dict(title="Entropy", len=1/len(schedules), y=1 - i/len(schedules)),
                showscale=(i == 0),
            ), row=i+1, col=1)
            fig.update_yaxes(title_text="Layer", row=i+1, col=1)
        ...
        _add_fig(f"logit_lens_entropy_{run_name}", fig,
    ...
    for analysis in analyses:
        ...
        for sched in schedules:
            ...
            fig.add_trace(go.Scatter(
                x=d["alphas"], y=d["mean_burst_acc_on_other"],
                name=sched, line=dict(color=c, width=2), mode="lines+markers",
                legendgroup=sched, showlegend=True,
            ), row=1, col=1)
            fig.add_trace(go.Scatter(
                x=d["alphas"], y=d["mean_other_acc_on_other"],
                name=sched, line=dict(color=c, width=2, dash="dash"), mode="lines+markers",
                legendgroup=sched, showlegend=False,
            ), row=1, col=2)
            fig.add_trace(go.Scatter(
                x=d["alphas"], y=d["mean_burst_acc_on_burst"],
                name=sched, line=dict(color=c, width=2, dash="dot"), mode="lines+markers",
                legendgroup=sched, showlegend=False,
            ), row=1, col=3)

        fig.update_xaxes(title_text="α (steering strength)", type="log")
        fig.update_layout(
            title=(f"Activation Steering: α Sweep at Layer {st[schedules[0]]['steer_layer']} — {run_name}"),
            template="plotly_white", height=500,
            legend_title="Schedule",
        )
        _add_fig(f"steering_alpha_sweep_{run_name}", fig,
                 f"Steering α Sweep — {run_name}")
    ...
    for analysis in analyses:
        ...
        for sched in schedules:
            ...
            fig.add_trace(go.Scatter(
                x=layer_labels, y=d["mean_burst_acc"],
                name=sched, line=dict(color=c, width=2), mode="lines+markers",
                legendgroup=sched, showlegend=True,
            ), row=1, col=1)
            fig.add_trace(go.Scatter(
                x=layer_labels, y=d["mean_other_acc"],
                name=sched, line=dict(color=c, width=2, dash="dash"), mode="lines+markers",
                legendgroup=sched, showlegend=False,
            ), row=1, col=2)
        ...
        _add_fig(f"steering_layer_sweep_{run_name}", fig,
    ...
    for analysis in analyses:
        ...
        readabilities = [ll.get(s, {}).get("with_ln", {}).get("overall_readability", float("nan"))
                         for s in schedules]
        ranks = [ll.get(s, {}).get("with_ln", {}).get("overall_mean_rank", float("nan"))
                 for s in schedules]
        max_steers = []
        for s in schedules:
            sd = st.get(s, {})
        ...
        _add_fig(f"summary_fingerprint_{run_name}", fig,
    ...
    for analysis in analyses:
        ...
        _add_fig(f"delta_norm_per_layer_{run_name}", fig,
...
<style>
  ...
  .metric-info .formula { font-family: monospace; background: #eef; padding: 4px 8px; border-radius: 4px; }
...
"""]
    ...
    for i, (key, title, fig) in enumerate(all_figs):
        desc = _METRIC_DESCRIPTIONS.get(key.split("_" + analyses[0]["run_name"])[0]
                                         if analyses else key, {})
        anchor = f"chart_{i}"
        ...
        if desc:
            ...
            if desc.get("what"):
                html_parts.append(f'<div class="what"><strong>What:</strong> {desc["what"]}</div>\n')
...
def _find_all_run_dirs(data_root: Path) -> list[Path]:
    candidates = sorted(
        ...
        if p.is_dir() and ("burst_" in p.name)
    )

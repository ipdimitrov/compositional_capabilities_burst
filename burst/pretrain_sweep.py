...
from burst.experiment import build_data, run_pretrain
...
def _run_group(args: tuple[tuple[int, int], list[SweepRun], dict[str, Any]]) -> list[dict[str, Any]]:
...
def _plot_summary(raw_df: pd.DataFrame, agg_df: pd.DataFrame, out_dir: Path) -> None:
    ...
    _bar_by(raw_df, "lr_pretrain_end_frac", "summary_by_lr_pretrain_end_frac.png", "lr_pretrain_end_frac")
    ...
    top = agg_df.sort_values(["final_acc_other_mean", "final_loss_mean"], ascending=[False, True]).head(20)
...
def _build_tables(raw_df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, pd.DataFrame]]:
    ...
    for param in ["lr", "lr_pretrain_end_frac", "beta1", "pre_burst_steps", "n_a", "n_docs_per_task"]:
        sdf = (
    ...
    for param, sdf in by_param.items():
        best = sdf.sort_values(["final_acc_other_mean", "final_loss_mean"], ascending=[False, True]).iloc[0]
        optimal_rows.append(
            {
                "parameter": param,
...
def _write_excel(
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        raw_export = raw_df.drop(columns=["curve_step", "curve_acc_other", "curve_acc_burst", "curve_loss"])
...
def main() -> None:
    ...
    tasks = _build_group_tasks(grouped, max_workers=max_workers, chunk_size=args.chunk_size, base_cfg=base_cfg)
    ...
    while pending:
        ...
        if retries_left <= 0:
            raise RuntimeError(f"{len(failed)} task chunks failed after retries. Last failure likely shown above.")
        ...
        if saw_cuda_oom and cur_workers > 1:
            ...
            if next_workers < cur_workers:
                print(f"CUDA OOM detected. Reducing workers: {cur_workers} -> {next_workers}", flush=True)

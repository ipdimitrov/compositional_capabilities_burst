...
def schedule_bars(
    ...
    axes[0].set_title(
        "Training Schedules: Fraction of Burst Data per Step",
        fontsize=14,
        fontweight="bold",
        pad=10,
    )
...
def auc_diff(
    ...
    ax.set_title(
        "Pairwise Reversion AUC Difference (%)\n(row - col) / |col| x 100",
        fontsize=14,
        fontweight="bold",
    )
...
def lr_schedule(pdir: Path, cfg: _Cfg) -> Path:
    ...
    for sched in _ordered(schedules):
        ...
        ax.plot(
            steps,
            lrs,
            color=PALETTE.get(sched, "#1565C0"),
            lw=2.5,
...
def reversion_zoom(
    ...
    _style(
        ...
        (
            "Forgetting Dynamics: Special Class Accuracy During Reversion\n"
            f"(mean +/- 95% CI, n={ns} seeds)"
        ),
    )
...
def summary_table(
    ...
    ax.set_title(
        f"Summary Statistics (n={len(groups[scheds[0]])} seeds per schedule)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def pairwise_grad_cosine_heatmap(
    ...
    for sched in scheds:
        ...
        for target_step in sorted(snaps_by_step.keys()):
            ...
            std_matrix = (
                np.std(stacked, axis=0) if len(matrices) > 1 else np.zeros_like(mean_matrix)
            )
            ...
            for show_error in [False, True]:
                ...
                ax.set_title(
                    (
                        f"Pairwise Grad Cosine Sim -- {sched_label} -- "
                        f"Step {target_step} ({phase}){err_note}\n"
...
def pairwise_grad_cosine_evolution_by_metric(
    ...
    for metric in metrics:
        ...
        for sched in scheds:
            ...
            n_s = len(vals_arr)
            ci = (
                1.96 * np.std(vals_arr, axis=0) / np.sqrt(n_s)
                if n_s > 1
                else np.std(vals_arr, axis=0)
            )
...
def pairwise_grad_cosine_evolution_per_schedule(
    ...
    fig.suptitle(
        "Pairwise Grad Cosine Metrics per Schedule\n(mean +/- 95% CI across seeds)",
        fontsize=13,
        fontweight="bold",
    )
    fig.tight_layout()
...
def probe_heatmap_aggregated(
    ...
    {**cfg.get("base_cfg", cfg), "_burst_mode": cfg.get("burst_mode", MODE_CURRENT)}
    T = meta["total_steps"]
...
def probe_layer_schedule_heatmap(
    ...
    for target_step, phase_label in [(T, "end_train"), (T + U, "end_reversion")]:
        ...
        ax.set_xticklabels(
            [SCHED_SHORT.get(s, s) for s in col_scheds], rotation=30, ha="right", fontsize=10
        )
...
def grad_cosine_layer_step_heatmap(
    ...
    for sched, layer_data in sched_layer_data.items():
        ...
        fig, ax = plt.subplots(figsize=(fig_w, fig_h))
        im = ax.imshow(
            grid, aspect="auto", cmap="RdBu_r", vmin=-1.0, vmax=1.0, interpolation="nearest"
        )
        ...
        ax.set_xticklabels(
            [str(int(steps_ref[i])) for i in tick_idxs], rotation=45, ha="right", fontsize=8
        )
        ax.set_yticks(range(n_layers))
        ...
        sched_label = SCHED_SHORT.get(sched, sched)
        ax.set_title(
            f"{sched_label}: Per-Layer Gradient Cosine Similarity Over Time\n"
...
def grad_cosine_layer_schedule_heatmap(
    ...
    for phase_label in phase_labels:
        ...
        ax.set_xticklabels(
            [SCHED_SHORT.get(s, s) for s in scheds], rotation=30, ha="right", fontsize=10
        )
        ...
        ax.set_ylabel("Layer", fontsize=11)
        ax.set_title(
            f"Layer x Schedule: Gradient Cosine Similarity ({phase_label})\n"
...
def grad_cosine_layer_change_heatmap(
    ...
    for sched, layer_data in sched_layer_data.items():
        ...
        ax.set_xticks(tick_idxs)
        ax.set_xticklabels(
            [str(int(mid_steps[i])) for i in tick_idxs], rotation=45, ha="right", fontsize=8
        )
        ax.set_yticks(range(n_layers))
        ax.set_yticklabels(layers_present, fontsize=9)
        ax.set_xlabel("Step", fontsize=11)
        ...
        sched_label = SCHED_SHORT.get(sched, sched)
        ax.set_title(
            f"{sched_label}: Rate of Change of Per-Layer Cosine Similarity\n"

...
def load_probe_results(run_dir: Path) -> tuple[list[dict], dict]:
    ...
    if not probe_dir.exists():
        ...
        print("Run the probe script first:")
    ...
    if not meta_path.exists():
        ...
        meta = {
            ...
            "token_labels": r0.get(
                "token_labels",
                [f"t{i}" for i in range(list(r0["probes"].values())[0]["train_acc_KT"].shape[1])],
            ),
            "n_layers": r0.get(
                "n_layers", list(r0["probes"].values())[0]["train_acc_KT"].shape[0] - 1
            ),
...
def plot_layer_schedule_heatmap(
    ...
    ax.set_title(
        f"Layer × Schedule — mean probe accuracy at step {step}", fontsize=13, fontweight="bold"
    )

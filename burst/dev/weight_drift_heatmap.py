        ...
        for r in jobs[sched][:n_seeds]:
            ...
            base_sd = {
                ...
                for k, v in torch.load(
                    str(files[sorted_steps[0]]), map_location="cpu", weights_only=True
                ).items()
            }
            if layers is None:
            ...
            for ci, step in enumerate(sorted_steps):
                ...
                for li, (gname, pnames) in enumerate(lg):
                    cum_row[li, ci] = (
                        sum((sd[p] - base_sd[p]).norm().item() ** 2 for p in pnames) ** 0.5
                    )
                    step_row[li, ci] = (
                        sum((sd[p] - prev_sd[p]).norm().item() ** 2 for p in pnames) ** 0.5
                    )
                prev_sd = sd
...
def _heatmap(z, x, y, title, colorscale, cbar_title, zmin=None, zmax=None, zmid=None):
    kw = dict(
        z=z, x=[str(s) for s in x], y=y, colorscale=colorscale, colorbar=dict(title=cbar_title)
    )
    if zmid is not None:
...
def _line_per_layer(steps, grid, layers, title, ylabel, log_y=False):
    ...
    layout = dict(
        title=title, xaxis_title="Step", yaxis_title=ylabel, template="plotly_white", height=450
    )
    if log_y:
...
def main():
    ...
    figs.append(
        (
            ...
            _line_per_schedule(
                wd, "stepwise", "Total ||W_t − W_{{t-1}}||_F", "||δW||_F", log_y=True
            ),
        )
    )
    ...
    for sched in sorted(gc, key=_sched_order):
        ...
        figs2.append(
            (
                f"{label}: Gradient Cosine Similarity (Layer × Step)",
                _heatmap(
                    d["grid"],
                    d["steps"],
                    d["layers"],

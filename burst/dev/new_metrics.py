    ...
    for sched in schedules:
        ...
        for i, r_src in enumerate(sched_results):
            ...
            r_tgt = next(
                (
                    r
                    for j, r in enumerate(sched_results)
                    if j != i and (ckpt_root / r["label"]).exists()
                ),
                None,
            )
            if r_tgt is None:
            ...
            peak_sd = {
                ...
                for k, v in torch.load(
                    str(files_src[peak_step]), map_location="cpu", weights_only=True
                ).items()
            }
            pre_sd = {
                k: v.float()
                for k, v in torch.load(
                    str(files_src[pre_step]), map_location="cpu", weights_only=True
                ).items()
            }
            tgt_sd = {
                k: v.float()
                for k, v in torch.load(
                    str(files_tgt[pre_step_tgt]), map_location="cpu", weights_only=True
                ).items()
            }
        ...
        for r in sched_results:
            ...
            for alpha in alphas:
                ...
                loss = F.cross_entropy(
                    logits.reshape(-1, logits.size(-1)), tgt_t.reshape(-1)
                ).item()
...
def compute_forgetting_speed_decomposition(all_results: list[dict]) -> dict:
    ...
    *depth* of final forgetting.
    ...
    for sched in schedules:
        ...
        results[sched] = {
            "mean_initial_slope": float(np.nanmean(initial_slopes))
            if initial_slopes
            else float("nan"),
...
def compute_burst_position_comparison(existing_analyses: list[dict]) -> dict:
    ...
    for analysis in existing_analyses:
        ...
        for sched in schedules:
            ...
            adl_acc_drop = (
                adl_sched[max(peak_steps)]["mean_max_acc_drop"] if peak_steps else float("nan")
            )
...
_METRIC_DESCRIPTIONS = {
    "task_vector_transfer": {
        ...
        "high": (
            "High transfer accuracy → the burst capability is stored as a modular wrapper "
            "(τ encodes it cleanly)."
        ),
        "low": (
            "Low transfer accuracy → the capability is entangled with the specific "
            "training trajectory and cannot be transplanted."
        ),
        "limitations": (
            "Transfer may fail for reasons other than depth (e.g., different random initialisation). "  # noqa: E501
    ...
    "forgetting_trajectory_dim": {
        ...
        "high": (
            "High dimensionality → the model is restructuring weights in many directions "
        ...
        "limitations": (
            "Sensitive to checkpoint frequency. Very small models may have low dimensionality by default. "  # noqa: E501
    ...
    "relearning_efficiency": {
        ...
        "what": (
            ...
            "and measures the burst accuracy recovery curve. AUC of this curve is the relearning score."  # noqa: E501
        ),
        "high": (
            "High AUC → fast reacquisition of burst capability (the pathway was "
            "suppressed, not destroyed — shallow)."
        ),
    ...
    "linear_mode_connectivity": {
        ...
        "high": (
            "High barrier → the two models are in different loss basins (deep learning "
            "found a genuinely different solution)."
        ),
        "low": (
            "Low barrier → the path between peak and reverted is smooth (shallow wrapper — "
            "model just slid back along a ridge)."
        ),
        "limitations": (
    ...
    "pruning_robustness": {
        ...
        "high": (
            "Robust to pruning (accuracy stays high at high sparsity) → capability is "
            "distributed across many weights (deep)."
        ),
        "low": (
            "Fragile to pruning (accuracy drops fast at low sparsity) → capability is "
            "concentrated in a few high-magnitude weights (shallow wrapper)."
        ),
        "limitations": (
            "Magnitude-based pruning is a crude proxy. Structured pruning or Fisher-information-based "  # noqa: E501
    ...
    "pairwise_grad_separation": {
        ...
        "what": (
            ...
            "task gradient from the pre-computed pairwise gradient snapshots at 5 key training steps "  # noqa: E501
            "(step 0, mid-burst, end-burst, mid-reversion, end-reversion)."
        ),
        "high": (
            "High cosine sim (near 1) → burst and other gradients are aligned (no conflict "
            "— model learns both simultaneously)."
        ),
        "low": (
            "Low cosine sim (near -1) → burst and other gradients are opposed (strong "
            "conflict — burst learning interferes with other-class knowledge)."
        ),
        "limitations": (
    ...
    "forgetting_speed_decomposition": {
        ...
        "high": (
            "High initial slope magnitude → fast initial forgetting. High plateau → "
            "capability partially retained at end."
        ),
        "low": (
            "Low initial slope → slow forgetting onset. Low plateau → capability fully "
    ...
    "layer_interference_localisation": {
        ...
        "high": (
            "High (near 1) cosine sim at a layer → burst and other gradients agree at that "
            "layer (shared representation)."
        ),
        "low": (
            "Low (near -1) cosine sim → strong conflict at that layer (burst is "
            "overwriting other-class knowledge there)."
        ),
        "limitations": (
            "burst_100 has zero interference by construction (pure burst batches), so all values "
            "will be near 1. The metric is most informative for mixed schedules (burst_10 to burst_75)."  # noqa: E501
        ),
    },
    "grad_interference_temporal": {
        ...
        "high": (
            "Fast re-alignment (low re-alignment step) → the burst representation is "
            "quickly suppressed (shallow)."
        ),
        "low": (
            "Slow re-alignment (high re-alignment step) → the burst representation "
            "persists and conflicts with other-class learning for longer (deep)."
        ),
        "limitations": (
            "The 0.1 threshold is empirically chosen based on observed sim ranges. "
            "Re-alignment speed is sensitive to the initial cosine similarity at the start of reversion. "  # noqa: E501
            "For burst_100 (pure burst batches), the starting sim is most negative, making re-alignment slower."  # noqa: E501
        ),
    },
    "grad_projection": {
        ...
        "high": (
            ...
            "High useful_learning → burst steps move model in directions orthogonal to other-class concerns."  # noqa: E501
        ),
        "low": (
            "Low interference_ratio → most of the burst gradient is orthogonal to other-class gradients "  # noqa: E501
    ...
    "burst_position_comparison": {
        ...
        "high": (
            "Higher reversion AUC at a given position → capability is more robust at that "
...
def make_dashboard(new_results: dict, existing_analyses: list[dict], out_dir: Path) -> None:
    ...
    for analysis in existing_analyses:
        ...
        fig.add_vline(x=499, line_dash="dash", line_color="gray", annotation_text="burst→reversion")
        fig.update_layout(
    ...
    for analysis in existing_analyses:
        ...
        fig.add_hline(
            y=0.5, line_dash="dot", line_color="gray", annotation_text="re-alignment threshold"
        )
        fig.update_layout(
    ...
    def _proj_timeseries_fig(
        ...
        if hline is not None:
            fig.add_hline(
                y=hline, line_dash="dot", line_color="gray", annotation_text="random baseline"
            )
        fig.update_layout(
            title=title,
            xaxis_title="Training Step",
    ...
    for analysis in existing_analyses:
        ...
        _add_fig(
            ...
            _proj_timeseries_fig(
                ...
                "<sup>||g_burst^parallel||: projection of burst gradient onto other-class direction. "  # noqa: E501
        ...
        _add_fig(
            ...
            _proj_timeseries_fig(
                ...
                "<sup>||g_burst^perp||: orthogonal component — safe learning that does not conflict.</sup>",  # noqa: E501
...
"""
    ...
    for i, (key, title, _) in enumerate(all_figs):
        ...
        badge = (
            '<span class="badge">needs checkpoints</span>'
            if needs_ckpt
            else '<span class="badge no-ckpt">from existing data</span>'
        )
        anchor = f"chart_{i}"
        html_parts.append(f'  <a href="#{anchor}">{i + 1}. {title}{badge}</a>\n')
    ...
    for i, (key, title, fig) in enumerate(all_figs):
        ...
        if desc:
            ...
            if desc.get("what"):
                html_parts.append(
                    f'<div class="what"><strong>What this measures:</strong> {desc["what"]}</div>\n'
                )
            ...
            if desc.get("limitations"):
                html_parts.append(
                    f'<div class="limits"><strong>Limitations:</strong> {desc["limitations"]}</div>\n'  # noqa: E501
                )
            html_parts.append("</div>\n")

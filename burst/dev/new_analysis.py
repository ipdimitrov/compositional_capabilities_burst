...
def plot_layerwise_weight_diff(data: dict, out_dir: Path, P: int = 0):
    ...
    _style(
        ax, "Step", "||W_step - W_pre||₂", "Total Weight Difference vs Pre-Burst (mean ± 95% CI)"
    )
    ...
    for sched in schedules:
        ...
        for li, layer in enumerate(layers):
            ...
            m = vals.mean(axis=0)
            ax.plot(steps_ref, m, color=cmap(li / max(len(layers) - 1, 1)), lw=1.5, label=layer)
...
def plot_layerwise_activations(data: dict, out_dir: Path):
    ...
    for sched in schedules:
        ...
        for data_type, data_key in [("burst", "burst_norms"), ("other", "other_norms")]:
            ...
            fig.tight_layout()
            fig.savefig(
                out_dir / f"activation_layers_{sched}_{data_type}.png", dpi=150, bbox_inches="tight"
            )
            plt.close(fig)
    ...
    for loss_type in ["burst", "other"]:
        ...
        _style(
            ...
            (
                ...
                f"{data[schedules[0]].get('n_directions', '?')} Random Directions"
            ),
        )
...
def plot_weight_norms(data: dict, out_dir: Path):
    ...
    ax.bar(xs, means, yerr=cis, color=colors, edgecolor="black", lw=0.8, capsize=5, alpha=0.85)
    ...
    _style(
        ax,
        "",
        "||W||₂ (total weight norm at peak burst)",
    ...
    if len(burst_pcts) > 2:
        ...
        _style(ax, "Burst %", "||W||₂", "Weight Norm vs Burst Percentage\n(each dot = one seed)")
        fig.tight_layout()
...
def plot_grad_norms_and_cosim(gs_records: list, out_dir: Path, P: int = 0):
    ...
    for norm_type, norm_label in [
        ...
        _style(ax, "Step", norm_label, f"Gradient Norm ({norm_label}) Over Training")
        ax.legend(fontsize=9, loc="best")
        fig.tight_layout()
    ...
    for sched in schedules:
        ...
        for r in runs:
            ...
            ax.scatter(
                cosims[:n], burst_norms[:n], color=_color(sched), s=15, alpha=0.4, edgecolor="none"
            )
    ...
    _style(
        ax, "Step", "Cosine Similarity", "Gradient Cosine Similarity Over Time (every checkpoint)"
    )
    ax.legend(fontsize=9, loc="best")
    fig.tight_layout()
...
def plot_grad_rank(gs_records: list, out_dir: Path):
    ...
    for sched in schedules:
        ...
        for li, layer in enumerate(layers):
            ...
            valid_mask = arr > 0
            m = np.where(
                valid_mask.any(axis=0), np.nanmean(np.where(valid_mask, arr, np.nan), axis=0), 0
            )
            ax.plot(steps_ref, m, color=cmap(li / max(len(layers) - 1, 1)), lw=1.5, label=layer)
    ...
    _style(ax, "Step", "Mean Effective Rank", "Mean Gradient Effective Rank Over Training")
    ax.legend(fontsize=9, loc="best")
    fig.tight_layout()
...
def plot_sharpness(loss_surface_data: dict, out_dir: Path):
    ...
    for loss_type in ["burst", "other"]:
        ...
        xs = np.arange(len(schedules))
        colors = [_color(s) for s in schedules]
        ax.bar(xs, vals, yerr=cis, color=colors, edgecolor="black", lw=0.8, capsize=5, alpha=0.85)
        ax.set_xticks(xs)
        ax.set_xticklabels([_label(s) for s in schedules], fontsize=9, rotation=30, ha="right")
        _style(
            ax,

...
from burst.core.train_utils import resolve_run_paths, load_net
...
def plot_sharpness(results: dict, out_dir: Path, n_directions: int):
    ...
    for sched in schedules:
        ...
        if n_seeds > 1 and n_per_seed > 0:
            seed_burst = [
                burst_sharp_per_dir[i * n_per_seed : (i + 1) * n_per_seed].mean()
                for i in range(n_seeds)
            ]
            seed_other = [
                other_sharp_per_dir[i * n_per_seed : (i + 1) * n_per_seed].mean()
                for i in range(n_seeds)
            ]
...
def main():
    ...
    print(f"Directions: {args.n_directions}, points: {args.n_points}, epsilon: ±{args.max_epsilon}")
    ...
    print(
        f"Estimated: {total_fwd} forward passes, ~{est_seconds:.0f}s ({est_seconds / 60:.1f} min)\n"
    )

    t0 = time.time()

    raw_results: dict[str, dict] = {s: {"burst": [], "other": [], "seeds": []} for s in schedules}
    ...
    if args.max_workers <= 1:
        for i, (sched, seed, ckpt_path, cfg) in enumerate(work_items):
            ...
            print(
                f"  [{i + 1}/{len(work_items)}] {sched} s{seed} done ({elapsed:.1f}s)", flush=True
            )
    else:
        with ProcessPoolExecutor(
            max_workers=args.max_workers, mp_context=multiprocessing.get_context("spawn")
        ) as pool:
            futures = {}
            ...
            for fut in as_completed(futures):
                ...
                elapsed = time.time() - t0
                print(
                    f"  [{done_count}/{len(work_items)}] {sched} s{seed} done ({elapsed:.1f}s)",
                    flush=True,
                )

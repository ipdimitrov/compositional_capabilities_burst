...
from burst.train_utils import resolve_run_paths, load_net
...
def compute_one_seed(
    ...
    """Run n_directions filter-normalised perturbations, return (burst_losses, other_losses).

    Each is a list of n_directions lists, each of len(epsilons) floats.
    """
    base_sd = {k: v.float().cpu() for k, v in
               torch.load(ckpt_path, map_location="cpu", weights_only=True).items()}
...
def plot_mean_and_variance(results: dict, out_dir: Path, n_directions: int):
    ...
    for loss_type in ["burst", "other"]:
        ...
        for sched in schedules:
            ...
            epsilons = d["epsilons"]
            arr = np.array(d[key])
            ax_mag.plot(epsilons, arr.mean(axis=0), color=_color(sched), lw=2, label=_label(sched))
            ax_var.plot(epsilons, arr.var(axis=0), color=_color(sched), lw=2, label=_label(sched))

        _style(ax_mag, "ε (perturbation)", "Mean Loss",
               f"Loss Basin: Mean Loss ({loss_type}) Across {n_directions} Random Directions")
        ax_mag.legend(fontsize=9, loc="best")
        fig_mag.tight_layout()
        fig_mag.savefig(out_dir / f"basin_mean_{loss_type}.png", dpi=150, bbox_inches="tight")
        plt.close(fig_mag)

        _style(ax_var, "ε (perturbation)", "Variance of Loss",
               f"Loss Basin: Variance ({loss_type}) Across {n_directions} Random Directions")
        ax_var.legend(fontsize=9, loc="best")
        fig_var.tight_layout()
        fig_var.savefig(out_dir / f"basin_var_{loss_type}.png", dpi=150, bbox_inches="tight")
        plt.close(fig_var)


def plot_sharpness(results: dict, out_dir: Path, n_directions: int):
    """Grouped bar chart: burst sharpness vs other sharpness per schedule, log y-axis.

    Sharpness per (schedule, seed, direction) = max(loss along eps) - loss at center.
    We average over directions per seed, then show mean +/- 95% CI across seeds.
    """
    schedules = sorted(results.keys(), key=_sched_order)
    ...
    for sched in schedules:
        ...
        burst_sharp_per_dir = burst_arr.max(axis=1) - burst_arr[:, center_idx]
        other_sharp_per_dir = other_arr.max(axis=1) - other_arr[:, center_idx]

        n_seeds = d.get("n_seeds", 1)
        n_per_seed = len(burst_sharp_per_dir) // max(n_seeds, 1)

        if n_seeds > 1 and n_per_seed > 0:
            seed_burst = [burst_sharp_per_dir[i * n_per_seed:(i + 1) * n_per_seed].mean()
                          for i in range(n_seeds)]
            seed_other = [other_sharp_per_dir[i * n_per_seed:(i + 1) * n_per_seed].mean()
                          for i in range(n_seeds)]
            burst_means.append(np.mean(seed_burst))
            other_means.append(np.mean(seed_other))
            burst_cis.append(1.96 * np.std(seed_burst) / np.sqrt(n_seeds))
            other_cis.append(1.96 * np.std(seed_other) / np.sqrt(n_seeds))
        else:
            burst_means.append(float(burst_sharp_per_dir.mean()))
            other_means.append(float(other_sharp_per_dir.mean()))
    ...
    ax.bar(xs - w / 2, burst_means, w, yerr=burst_cis, label="Burst",
           color="#E91E63", edgecolor="black", lw=0.6, capsize=4, alpha=0.85)
    ax.bar(xs + w / 2, other_means, w, yerr=other_cis, label="Other",
           color="#2196F3", edgecolor="black", lw=0.6, capsize=4, alpha=0.85)
    ax.set_yscale("log")
    ax.set_xticks(xs)
    ax.set_xticklabels([_label(s) for s in schedules], fontsize=9, rotation=30, ha="right")
    _style(ax, "", "Sharpness (max − center loss, log scale)",
           f"Basin Sharpness: Burst vs Other ({n_directions} directions, mean ± 95% CI)")
    ax.legend(fontsize=11)
    fig.tight_layout()
    fig.savefig(out_dir / "sharpness_comparison.png", dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(
        description="50-direction filter-normalised loss basin analysis at peak burst.")
    ...
    burst_eval = burst_docs[rng.choice(burst_docs.shape[0], n_burst, replace=False)]
    other_eval = other_docs[rng.choice(other_docs.shape[0], n_other, replace=False)]
    ...
    epsilons = np.linspace(-args.max_epsilon, args.max_epsilon, args.n_points)
    ...
    for sched in schedules:
        for r in jobs_by_schedule[sched]:
            ...
            work_items.append((sched, r["seed"], str(files[peak_step]), r["config"]))

    print(f"Run: {run_dir.name}")
    print(f"Schedules: {len(schedules)}, work items: {len(work_items)}")
    print(f"Directions: {args.n_directions}, points: {args.n_points}, "
          f"epsilon: ±{args.max_epsilon}")
    print(f"Workers: {args.max_workers}")
    print(f"Eval docs: {n_burst} burst, {n_other} other")

    per_item_fwd = args.n_directions * args.n_points * 2
    total_fwd = len(work_items) * per_item_fwd
    est_seconds = total_fwd * 0.003
    print(f"Estimated: {total_fwd} forward passes, ~{est_seconds:.0f}s "
          f"({est_seconds / 60:.1f} min)\n")

    t0 = time.time()

    raw_results: dict[str, dict] = {s: {"burst": [], "other": [], "seeds": []}
                                    for s in schedules}

    if args.max_workers <= 1:
        for i, (sched, seed, ckpt_path, cfg) in enumerate(work_items):
            burst_losses, other_losses = compute_one_seed(
                cfg, ckpt_path, burst_eval, other_eval, args.n_directions, epsilons)
            ...
            print(f"  [{i + 1}/{len(work_items)}] {sched} s{seed} done ({elapsed:.1f}s)",
                  flush=True)
    else:
        with ProcessPoolExecutor(max_workers=args.max_workers,
            ...
            for sched, seed, ckpt_path, cfg in work_items:
                fut = pool.submit(compute_one_seed, cfg, ckpt_path,
                                  burst_eval, other_eval, args.n_directions, epsilons)
            ...
            for fut in as_completed(futures):
                ...
                done_count += 1
                elapsed = time.time() - t0
                print(f"  [{done_count}/{len(work_items)}] {sched} s{seed} done "
                      f"({elapsed:.1f}s)", flush=True)

    compute_time = time.time() - t0
    print(f"\nCompute done in {compute_time:.1f}s ({compute_time / 60:.1f} min)")

...
"""
import sys, os, pickle, json, base64, traceback, argparse, time
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
import numpy as np
from pathlib import Path
from collections import defaultdict
from burst.pres_charts import PALETTE, SCHED_SHORT, _ordered, _group, generate_all, load_grad_sim_data, _group_gs
...
def _collect_analysis_charts(run_dir: Path, n_seeds: int = 3) -> dict[str, list[tuple[str, Path]]]:
    ...
    try:
        ...
        r = ua_analyse(run_dir, n_seeds=n_seeds, n_prune_levels=10,
                       relearn_steps=50, frank_seeds=3, xfrank_seeds=3, subsample_n=256)
        run_name = r["run_name"]
        metric_keys = [
        ...
        print(f"  [unified_analysis] done in {time.time()-t0:.1f}s ({len(out['unified'])} charts)", flush=True)
    except Exception:
    ...
    try:
        ...
        print(f"  [basin_metrics] done in {time.time()-t0:.1f}s ({len(out['basin'])} charts)", flush=True)
    except Exception:
...
def _build_txt(rd: Path, res: list, cfg: dict, cp: dict, gs_records: list,
               analysis_charts: dict[str, list[tuple[str, Path]]] | None = None) -> str:
    ...
    if gs_records:
        ...
        for sched in sc:
            ...
            for r in runs:
                ...
                seed = r.get("seed", "?")
                lines.append(f"    seed={seed}: steps={steps[:5]}...{steps[-3:] if len(steps)>5 else ''}")
                lines.append(f"              sims ={[f'{s:.3f}' for s in sims[:5]]}...{[f'{s:.3f}' for s in sims[-3:]] if len(sims)>5 else ''}")
            ...
            if all_end_burst:
                ...
                lines.append(f"    End-of-burst cossim: {arr.mean():.4f} +/- {arr.std():.4f} (n={len(arr)})")
            lines.append("")

        lines.append("PAIRWISE GRADIENT SIMILARITY SNAPSHOTS")
        lines.append("(task groups: BURST, O_F{i} = other tasks by function at burst_pos, ALL_OTHER, ALL_DATA)")
        lines.append("")
        for sched in sc:
            if sched not in gs_groups:
                continue
            ...
            for step in sorted(snaps_by_step.keys()):
                ...
                lines.append(f"  {sched} step={step} phase={snaps[0].get('phase','?')} (n={len(matrices)} seeds)")
    ...
    lines.append("")
    chart_descs = [
        ("Schedule Bars", "Fraction of burst-class data per training step for each schedule. Shows the temporal structure of data exposure."),
        ("Special Class Accuracy Overlay", "Mean +/- 95% CI of burst-class free-generation accuracy over training steps, one line per schedule. Shows acquisition speed and forgetting dynamics."),
        ("Other Classes Accuracy Overlay", "Mean +/- 95% CI of other-class accuracy. Should remain near 1.0 throughout — measures catastrophic forgetting of background knowledge."),
        ("Reversion Zoom", "Burst-class accuracy during reversion phase only, aligned to reversion start. Directly compares forgetting speed across schedules."),
        ("Peak Burst Bars", "Bar chart of peak burst-class accuracy by schedule. All should be near 1.0 if the model has sufficient capacity."),
        ("Reversion AUC Bars", "Area under the burst-class accuracy curve during reversion. Higher = slower forgetting = more robust representation."),
        ("Life Bars", "Steps until burst accuracy drops to X% of peak. Lower = faster forgetting."),
        ("AUC Diff Heatmap", "Pairwise percentage difference in reversion AUC between schedules. Shows relative forgetting resistance."),
        ("Gradient Cosine Similarity Overlay", "Cosine similarity between burst-class and other-class gradient vectors over training. High similarity = integrated representations; low/negative = conflicting gradients."),
        ("Gradient Cosine End-of-Burst Bars", "Snapshot of gradient alignment at end of burst phase. Predicts forgetting speed."),
        ("Gradient Cosine vs AUC Scatter", "Correlation between end-of-burst gradient alignment and reversion AUC. Positive correlation validates the gradient-conflict theory of forgetting."),
        ("Pairwise Gradient Heatmaps", "Full pairwise cosine similarity matrix between task groups (BURST, O_F1..O_Fn, ALL_OTHER, ALL_DATA) at key training steps."),
        ("Per-Layer Gradient Cosine Heatmap", "Layer x Step heatmap showing which layers have aligned vs conflicting gradients over time."),
        ("Per-Layer End-of-Burst Bars", "Grouped bars: each layer group x each schedule at end of burst. Shows which layers drive gradient alignment."),
        ("ADL Delta Norm", "Magnitude of activation bias (checkpoint - pre-burst) on other-class inputs. Rising norm = global activation shift from burst learning."),
        ("ADL Readability", "Fraction of top-10 unembedding tokens that are burst-relevant. High = wrapper representation; low = deeper encoding."),
        ("ADL Causal Ablation", "Accuracy drop when projecting out the activation bias. Large drop = model relies on global bias (wrapper); small drop = deeper representation."),
        ("Probe Heatmaps", "Binary linear probe accuracy (other vs burst) at each (layer, token position) pair. Shows where and when the model develops burst-specific representations."),
    ]
    ...
    if analysis_charts:
        ...
        for section_name, tmp_name in [("Unified Analysis", "_unified_tmp"),
...
def build(rd, res, cfg, cp, analysis_charts=None):
    ...
    parts.append(f"<html><head><meta charset='utf-8'><title>Burst Report — {rd.name}</title><style>{_CSS}</style></head><body>")
    ...
    def _setup():
        ...
        parts.append(f"<li><b>Other Classes:</b> {n_a} bijections per position x {depth} positions = {n_a**depth} other-class compositions</li>")
    ...
    def _protocol():
        ...
        parts.append(f"<p>Other classes + Special class mixed per schedule.</p>")
    ...
    def _grad_sim():
        ...
        if not has_gs:
            return
        parts.append(_section("Gradient Cosine Similarity: Special vs Other Classes", anchor="grad-sim"))
        parts.append(
            "<p>Cosine similarity between the full-parameter gradient vectors computed on burst-class "
    ...
    def _pairwise_evo():
        has_pw = cp.get("pairwise_evo_by_metric") or cp.get("pairwise_evo_per_schedule") or cp.get("pairwise_heatmaps")
        if not has_pw:
            return
        ...
        parts.append(
            ...
            "BURST = all burst-class tasks; O_F{i} = other-class tasks grouped by function at burst position; "
    ...
    def _adl():
        ...
        parts.append(
            ...
            "Logit Lens readability tests whether the bias encodes burst-relevant tokens (wrapper hypothesis). "
    ...
    if analysis_charts:
        ...
        _try(lambda: _analysis_section("unified", "Unified Analysis", "unified"), "Unified Analysis")
        _try(lambda: _analysis_section("extended", "Extended Metrics", "extended"), "Extended Metrics")
        _try(lambda: _analysis_section("basin", "Basin Geometry Metrics", "basin"), "Basin Geometry")
    ...
    def _conclusions():
        ...
        for b, t in [
            ...
            ("Gradient alignment:", "Higher burst-vs-other gradient cosine similarity at end of burst predicts slower forgetting."),
...
def main():
    parser = argparse.ArgumentParser(description="Build combined HTML + TXT report for a burst experiment run.")
    parser.add_argument("run_dir", type=Path, nargs="?", default=None)
    parser.add_argument("--full", action="store_true",
                        help="Also run unified/basin/extended analysis (slower)")
    parser.add_argument("--n-seeds", type=int, default=3,
                        help="Number of seeds for unified/basin analysis")
    ...
    if pkl_path.exists():
        ...
        for j in cfg.get("jobs", []):
            results.append({
                ...
                "schedule": j["schedule"],
                "seed": j["seed"],
                "config": {**base, "total_steps": j.get("total_steps", base.get("total_steps", 500)),

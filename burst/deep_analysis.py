...
Dimension key:
    ...
    S: n_schedules
"""
import sys, os, argparse, pickle, json, time, multiprocessing
...
def _compute_delta_KTN(
    ...
    delta_KTN = torch.stack([
        (acts_ckpt[k] - acts_pre[k]).mean(dim=0) for k in range(K)
    ])
...
def compute_adl_for_label(
    ...
    ckpt_files = {int(p.stem.split("_")[1]): p
...
def extract_grad_interference(result: dict) -> dict:
    ...
    burst_steps = [s for s, p in zip(steps, phases) if p == PHASE_BURST]
    burst_sims = [v for v, p in zip(burst_vs_other, phases) if p == PHASE_BURST]
    rev_steps = [s for s, p in zip(steps, phases) if p == PHASE_REVERSION]
    ...
    for sched in schedules_present:
        ...
        for r in sched_results:
            ...
            ema = ema_interpolation_probe(
                ckpt_peak, ckpt_rev, cfg, burst_docs_BL, prompt_len,
            ...
            seeds_done += 1
            print(f"  {label}: EMA cliff_alpha={ema['cliff_alpha']:.2f}, "
                  f"auc={ema['auc']:.3f}, total_rank={dr['total_rank']}", flush=True)
        ...
        analysis["task_vectors"][sched] = {
            ...
            "mean_cliff_alpha": float(np.mean(ema_cliff_alphas)) if ema_cliff_alphas else float("nan"),
    ...
    for sched in schedules_present:
        ...
        for r in sched_results:
            ...
            trace = compute_critical_sharpness(
                str(ckpt_files[peak_step]), cfg, burst_docs_BL,
                n_samples=128, n_hutchinson=n_hutchinson,
            )
            traces.append(trace)
            seeds_done += 1
    ...
    for sched in schedules_present:
        ...
        for r in sched_results:
            ...
            for step_data in adl_result["adl_steps"]:
                step = step_data["step"]
                adl_by_step.setdefault(step, {
                    "readability": [], "max_acc_drop": [],
                    "delta_norm": [], "acc_baseline": [],
                })
                ...
                adl_by_step[step]["delta_norm"].append(
                    float(np.mean(step_data["delta_norm_K"])))
                adl_by_step[step]["acc_baseline"].append(step_data["acc_baseline"])

            seeds_done += 1
            peak_step_data = [s for s in adl_result["adl_steps"] if s["step"] == 499
                               or s["step"] == max(s2["step"] for s2 in adl_result["adl_steps"]
                                                    if s2["step"] <= 499)]
            if peak_step_data:
                ps = peak_step_data[-1]
                print(f"  {label}: readability@peak={np.mean(ps['mean_readability_K']):.3f}, "
                      f"max_drop={ps['max_acc_drop']:.3f}", flush=True)
...
def make_dashboard(analyses: list[dict], out_dir: Path) -> None:
    ...
    for analysis in analyses:
        ...
        for sched in schedules:
            ...
            fig1.add_trace(go.Scatter(
                x=steps_arr, y=mean_sims,
                name=f"{sched} ({run_name})",
                line=dict(color=_color(sched), width=2),
                mode="lines",
            ))
    fig1.add_vline(x=500, line_dash="dash", line_color="gray",
    ...
    for analysis in analyses:
        ...
        for sched in schedules:
            ...
            fig4.add_trace(go.Scatter(
                x=data["alphas"], y=data["mean_accs"],
                name=sched,
                line=dict(color=_color(sched), width=2),
                mode="lines+markers",
            ))
    ...
    # -----------------------------------------------------------------------
    fig5 = make_subplots(rows=1, cols=len(analyses),
                         subplot_titles=[a["run_name"] for a in analyses])
    for col_idx, analysis in enumerate(analyses, start=1):
        tv = analysis["task_vectors"]
        sm = analysis["summary_metrics"]
    ...
    # -----------------------------------------------------------------------
    fig6 = make_subplots(rows=1, cols=len(analyses),
                         subplot_titles=[a["run_name"] for a in analyses])
    for col_idx, analysis in enumerate(analyses, start=1):
    ...
    for analysis in analyses:
        ...
        sample_groups = list(next(iter(wdr.values()))["mean_rank_by_group"].keys())
        fig8 = go.Figure()
        for sched in schedules:
            data = wdr[sched]
    ...
    # -----------------------------------------------------------------------
    fig9 = make_subplots(rows=1, cols=len(analyses),
                         subplot_titles=[a["run_name"] for a in analyses])
    for col_idx, analysis in enumerate(analyses, start=1):
        wdr = analysis["weight_delta_rank"]
        sm = analysis["summary_metrics"]
    ...
    for analysis in analyses:
        ...
        for sched in schedules:
            ...
            fig12.add_trace(go.Scatter(
                x=steps_sorted, y=norms,
                name=sched,
                line=dict(color=_color(sched), width=2),
                mode="lines+markers",
            ))
        fig12.add_vline(x=499, line_dash="dash", line_color="gray",
                        annotation_text="burst→reversion")
        fig12.update_layout(
...
def main():
    ...
    parser.add_argument("--adl-seeds", type=int, default=3,
                        help="Seeds per schedule for ADL + EMA probe (default: 3)")
    parser.add_argument("--adl-samples", type=int, default=256,

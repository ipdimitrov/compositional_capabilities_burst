...
def _build_layer_matrix(results, key):
    """Extract a per-layer-dict log key into a (steps, layers) matrix."""
    log = results["log"]
...
def plot_grad_cossim_x_norm_per_layer(ft_results, fg_results, figsize_per=(14, 3.5), same_vmax=True):
    ...
    for i, (ft, fg) in enumerate(pairs):
        ax = axes[i, 0]
        built_ft = _build_layer_matrix(ft, key)
        built_fg = _build_layer_matrix(fg, key) if fg is not None else None
...
def plot_grad_cossim_x_norm(ft_results, fg_results, figsize=(14, 5)):
    """Gradient cosine × norm on a continuous finetune → forget axis.

    Left: cossim × burst-grad-norm (finetune only — no burst training during forget).
    Right: cossim × bg-grad-norm (finetune) → cossim × bg-grad-norm (forget).
    """
    fig, (ax_burst, ax_bg) = plt.subplots(1, 2, figsize=figsize)
    drawn_boundary = False
    for ft, fg in _pair_ft_fg(ft_results, fg_results):
        color = _frac_color(ft["burst_frac"])
        ft_log = ft["log"]

        if "grad_cossim_x_norm_burst" in ft_log and ft_log["grad_cossim_x_norm_burst"]:
            ax_burst.plot(ft_log["step"], ft_log["grad_cossim_x_norm_burst"],
                          color=color, linewidth=2, label=ft["tag"])

        if "grad_cossim_x_norm_bg" in ft_log and ft_log["grad_cossim_x_norm_bg"]:
            ax_bg.plot(ft_log["step"], ft_log["grad_cossim_x_norm_bg"],
                       color=color, linewidth=2, label=ft["tag"])
        if fg is not None:
            fg_log = fg["log"]
            if "grad_cossim_x_norm" in fg_log and fg_log["grad_cossim_x_norm"]:
                fg_steps = _offset_fg_steps(ft_log, fg_log)
                ax_bg.plot(fg_steps, fg_log["grad_cossim_x_norm"],
                           color=color, linewidth=2)
    ...
    ax_burst.axhline(0, color="black", linewidth=0.5, linestyle=":")
    ax_burst.set_xlabel("Step"); ax_burst.set_ylabel("cos(g_burst, g_bg) × ‖g_burst‖")
    ax_burst.set_title("Gradient CosSim × Burst Norm (finetune)")
    ax_burst.legend(fontsize=7); ax_burst.grid(True, alpha=0.3)

    ax_bg.axhline(0, color="black", linewidth=0.5, linestyle=":")
    ax_bg.set_xlabel("Step (finetune | forget)")
    ax_bg.set_ylabel("cos(g_burst, g_bg) × ‖g_bg‖")
    ax_bg.set_title("Gradient CosSim × BG Norm (continuous)")
    ax_bg.legend(fontsize=7); ax_bg.grid(True, alpha=0.3)
...
def plot_fisher_overlap(ft_results, figsize=(16, 10)):
    """Fisher overlap of the burst gradient with the pretrained Fisher subspace.

    Top-left : global fisher_overlap time series, one line per burst fraction.
    Top-right : per-Fisher-direction breakdown (stacked area) for burst_100.
    Bottom   : per-block fisher_overlap heatmap (x=step, y=block, color=overlap)
               for the most concentrated run (burst_100 or highest available).
    """
    ft_list = ft_results if isinstance(ft_results, list) else [ft_results]

    # filter to runs that have Fisher data
    ft_fisher = [r for r in ft_list
                 if "fisher_overlap" in r["log"] and r["log"]["fisher_overlap"]]
    if not ft_fisher:
        fig, ax = plt.subplots(figsize=(6, 4))
        ax.text(0.5, 0.5, "No Fisher overlap data", ha="center", va="center",
    ...
    # ── top-left: global overlap time series ──────────────────────────────
    ax_ts = fig.add_subplot(gs[0, 0])
    for r in ft_fisher:
        ax_ts.plot(r["log"]["step"], r["log"]["fisher_overlap"],
                   color=_frac_color(r["burst_frac"]), linewidth=2, label=r["tag"])
    ax_ts.set_xlabel("Step"); ax_ts.set_ylabel("Fisher Overlap")
    ax_ts.set_title("Global Fisher Overlap (burst gradient)")
    ax_ts.set_ylim(bottom=0); ax_ts.legend(fontsize=7); ax_ts.grid(True, alpha=0.3)

    # ── top-right: per-direction breakdown for the most concentrated run ──
    ax_dir = fig.add_subplot(gs[0, 1])
    ref = max(ft_fisher, key=lambda r: r["burst_frac"])
    per_dir_log = ref["log"]["fisher_overlap_per_dir"]
    if per_dir_log and per_dir_log[0] is not None:
        steps = ref["log"]["step"][:len(per_dir_log)]
        per_dir_arr = np.array(per_dir_log)   # (T, k)
        k = per_dir_arr.shape[1]
        colors_dir = plt.cm.viridis(np.linspace(0.1, 0.9, k))
        bottom = np.zeros(len(steps))
        for i in range(k):
            ax_dir.fill_between(steps, bottom, bottom + per_dir_arr[:, i],
                                alpha=0.75, color=colors_dir[i],
                                label=f"v{i+1}")
            bottom += per_dir_arr[:, i]
        ax_dir.set_xlabel("Step"); ax_dir.set_ylabel("Overlap per Fisher direction")
        ax_dir.set_title(f"Per-Direction Fisher Overlap ({ref['tag']})")
        ax_dir.legend(fontsize=7, loc="upper right"); ax_dir.grid(True, alpha=0.3)
    else:
        ax_dir.set_title("Per-Direction data unavailable")

    # ── bottom: per-block heatmap for the most concentrated run ──────────
    ax_heat = fig.add_subplot(gs[1, :])
    per_layer_log = ref["log"].get("fisher_overlap_per_layer", [])
    if per_layer_log and per_layer_log[0]:
        layer_names = sorted(per_layer_log[0].keys())
        steps_pl = ref["log"]["step"][:len(per_layer_log)]
        matrix = np.array([[rec.get(l, 0.0) for l in layer_names]
                            for rec in per_layer_log])
        im = ax_heat.imshow(matrix.T, aspect="auto", cmap="YlOrRd", vmin=0,
                            extent=[steps_pl[0], steps_pl[-1],
                                    len(layer_names) - 0.5, -0.5])
        ax_heat.set_yticks(range(len(layer_names)))
        ax_heat.set_yticklabels(layer_names, fontsize=7)
        fig.colorbar(im, ax=ax_heat, fraction=0.02, pad=0.01,
                     label="Fisher Overlap")
        ax_heat.set_xlabel("Step")
        ax_heat.set_title(f"Per-Block Fisher Overlap ({ref['tag']})")
    else:
        ax_heat.set_title("Per-layer data unavailable")

    fig.suptitle("Fisher Overlap: Burst Gradient vs Pretrained Fisher Subspace",
                 fontsize=13, fontweight="bold")
    return fig
...
def save_report(pt, ft_results, fg_results, out_dir, analysis=None, prefix="report"):
    ...
    _save(plot_grad_cossim_x_norm(ft_list, fg_list), "08_grad_cossim_x_norm")
    _save(plot_grad_cossim_x_norm_per_layer(ft_list, fg_list), "09_grad_cossim_x_norm_per_layer")
    _save(plot_fisher_overlap(ft_list), "10_fisher_overlap")

    if analysis is not None:
        _save(plot_per_layer_drift(analysis, "pt_ft"), "10_layer_drift_finetune")
        _save(plot_per_layer_drift(analysis, "ft_fg"), "11_layer_drift_forget")
        _save(plot_svd_analysis(analysis), "12_svd_analysis")
        _save(plot_cka_matrices(analysis), "13_cka_matrices")
        _save(plot_summary_dashboard(ft_list, fg_list, analysis), "14_summary_dashboard")

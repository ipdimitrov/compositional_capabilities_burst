...
from simple.interp import (
    ...
    _get_grad_vector, gradient_cosine_and_norms_per_layer, grad_norm_entropy,
    ...
    for s in pbar:
        ...
        if s % eval_every == 0 or s == steps - 1:
            ...
            if _prev_g_burst is not None:
                gc_burst_succ = _F.cosine_similarity(
                    g_burst.unsqueeze(0), _prev_g_burst.unsqueeze(0)).item()
                gc_bg_succ = _F.cosine_similarity(
            ...
            # gradient effective rank: bg (=training), burst-only
            def _sample_bg_batch():
                _per = batch_size // len(bg_ids)
                _rem = batch_size % len(bg_ids)
                _parts = []
                for _i, _tid in enumerate(bg_ids):
                    _k = _per + (1 if _i < _rem else 0)
                    if _k > 0:
                        _idx = np.random.randint(len(bg_pool[_tid]), size=_k)
                        _parts.append(bg_pool[_tid][_idx])
                return np.concatenate(_parts)[np.random.permutation(batch_size)]
            def _sample_burst_batch():
                _idx = np.random.randint(len(eval_burst), size=min(batch_size, len(eval_burst)))
                return eval_burst[_idx]
            eff_rank_bg, pca_k_bg, _ = gradient_effective_rank(net, _sample_bg_batch,
                                                               n_batches=grad_eff_rank_batches)
            eff_rank_burst, pca_k_burst, _ = gradient_effective_rank(net, _sample_burst_batch,
                                                                      n_batches=grad_eff_rank_batches)
            log["grad_effective_rank"].append(eff_rank_bg)
